  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Clear the form but keep feedback
        setText("");
        setSelectedMoodKey(null);
        try {
          localStorage.removeItem(STORAGE_KEY);
        ...
        {lastFeedback && (
          <div className="mm-card mm-feedback-card">
            <div className="mm-feedback-header">
              <h3 className="mm-feedback-title">✨ AI Insights</h3>
              {lastFeedback.predictedMood && (
                <div className="mm-feedback-mood">
                  Predicted Mood: <span className="mm-feedback-mood-value">{lastFeedback.predictedMood.toFixed(1)}/5.0</span>
                </div>
              )}
            </div>
            <p className="mm-feedback-text">{lastFeedback.feedback}</p>
          </div>
        )}
      </div>
    </section>
