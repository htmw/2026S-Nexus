...
async def therapist_patient_journal_entries(
    ...
    # Endpoint is read-only and blocked if sharing is disabled or therapist is unlinked.
    await require_therapist(user)
...
async def therapist_patient_mood_trend(
    ...
    # Returns only analyzed points and naturally yields empty list when none are available.
    await require_therapist(user)
